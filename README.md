...
| 大类 | 二级 | 做什么 |
|---|---|---|
| **数据科技** | AI | 聚合 AI 新闻，分类、中文解读后推送飞书 |
| **国家政策** | 农民工 | 只索引中国政府网公开 RSS 中与农民工相关的标题，点开跳转官网原文 |

无服务器、无数据库、无月费——GitHub Actions + GitHub Pages。

## 国家政策 · 农民工（合规边界）

该频道刻意做得很克制，降低法律与安全风险：

- **只用官方 RSS**（中国政府网 `pushinfo`），抓取前检查 `robots.txt`
- **不登录、不绕过反爬、不镜像 PDF/正文**
- 本地只存：标题、发布时间、官方 URL、来源、命中关键词
- **不做 AI 解读**；飞书卡片只有标题 +「打开政府网站原文」
- 低频：每天约 2 次（北京时间 09:00 / 18:00）

不能保证绝对无纠纷；定位是公开信息索引与跳转，有疑义以官网原文为准。
...
自定义机器人只能在**群聊**里添加，工作台和开发者后台都不是入口。

建一个只有自己的群 → 右上角 `···` → 设置 → 群机器人 → 添加机器人 → 选「自定义机器人」→
填个名字 → 复制弹出的 webhook 地址。安全设置建议勾「签名校验」，把密钥一并记下。

地址应该长这样：

```
https://open.feishu.cn/open-apis/bot/v2/hook/a1b2c3d4-5e6f-7890-abcd-ef1234567890
```

如果拿到的是 `applink.feishu.cn` 开头的链接，那是应用分享链接，说明加成了「应用机器人」，
不能用。复制好之后本地先验一下通不通：

```bash
FEISHU_WEBHOOK='粘贴地址' npm run test-push
```
